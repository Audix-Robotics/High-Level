...
SENSOR_NAMES = tuple(STATE_TOPICS.keys())
...
class ArenaIrStateAdapter(Node):
    def __init__(self):
        ...
        self.declare_parameter('ir_trip_distance_front_left', 0.08)
        self.declare_parameter('ir_trip_distance_front_right', 0.08)
        self.declare_parameter('ir_trip_distance_left', 0.08)
        self.declare_parameter('ir_trip_distance_right', 0.08)
        self.declare_parameter('ir_trip_distance_back', 0.08)
        for sensor_name in SENSOR_NAMES:
            self.declare_parameter(f'source_sensor_for_{sensor_name}', sensor_name)
        ...
        for logical_name in SENSOR_NAMES:
            source_name = str(self.get_parameter(f'source_sensor_for_{logical_name}').value).strip()
            if source_name not in SENSOR_NAMES:
                raise ValueError(f'Invalid source sensor "{source_name}" for logical sensor "{logical_name}"')
            self.logical_sources[logical_name] = source_name

        self.blocked_state = {name: False for name in SENSOR_NAMES}
        self.last_update_sec = {name: None for name in SENSOR_NAMES}
        ...
        mapping = ', '.join(f'{logical}<-{source}' for logical, source in self.logical_sources.items())
        self.get_logger().info(f'IR state adapter ready. source_type={self.source_type}, mapping={mapping}')
    ...
    def _effective_blocked_state(self, sensor_name):
        source_name = self.logical_sources[sensor_name]
        last_update = self.last_update_sec[source_name]
        if last_update is None:
            return False
        if self._now_sec() - last_update > self.source_timeout_sec:
            return False
        return self.blocked_state[source_name]
